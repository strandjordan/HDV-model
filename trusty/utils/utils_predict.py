...
import openpifpaf
import os
import openpifpaf.plugins.apollocar3d
...
from openpifpaf.plugins.apollocar3d import constants as apollo_constants
...
"""COCO_PERSON_SKELETON = [
    ...
    [2, 4], [3, 5], [4, 6], [5, 7]]
"""
CAR_SKELETON = [apollo_constants.CAR_SKELETON_24]
#COCO_HEAD = [
#    [3,4]
#]
def convert(data):
    ...
    return X, Y, C, A"""
    # Case 1: already in [X, Y, C] form
    if isinstance(data, (list, tuple)) and len(data) == 3 and all(hasattr(d, '__len__') for d in data):
        X = list(data[0])
        Y = list(data[1])
        C = list(data[2])
    else:
        # Case 2: flat list of [x,y,c,...]
        arr = np.asarray(data, dtype=float).reshape(-1, 3)
        X = arr[:, 0].tolist()
        Y = arr[:, 1].tolist()
        C = arr[:, 2].tolist()

    # build flat [x,y,c,...] without np.array([X,Y,C])
    A = []
...
def draw_skeleton(img, kps, color, skeleton=CAR_SKELETON):
    X, Y, C, _ = convert(kps)
    """linewidth = 4
